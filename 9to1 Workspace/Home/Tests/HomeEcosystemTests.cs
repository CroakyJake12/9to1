...
public sealed class HomeEcosystemTests
{
    ...
    public async Task Library_filters_permissions_paginates_and_rechecks_before_open()
    {
        ...
        var service = new HomeLibraryService(index, authorization, router, new FakePreferences());
        ...
        Assert.Equal("1", router.Opened!.ObjectId);
    }
    ...
    public async Task Events_keep_authoritative_source_provenance_and_never_invent_records()
    {
        var sourceEvent = new HomeActivityEvent("event-1", DateTimeOffset.UtcNow, "app.one", "ArtifactChanged",
            "caller-id", "Caller", [new HomeDeepLink(HomeRoute.Library, "app.one", "artifact-1")], "info", "artifact", "Artifact updated.");
    ...
    [Fact]
    public void Navigation_retains_destination_state_and_can_open_deep_links()
    {
        var state = new HomeNavigationState();
        state.Navigate(HomeRoute.Library);
        state.State(HomeRoute.Library).Query = "report";
        Assert.True(state.OpenDeepLink(new HomeDeepLink(HomeRoute.Events, "app.one", "event-1"), new FakeRouter()));
    ...
    private static HomeArtifactReference Artifact(string app, string id, string name, bool pinned) => new(
        ...
        false, false, [], new HomeDeepLink(HomeRoute.Library, app, id));
    ...
    {
        public HomeTileProviderDescriptor Descriptor { get; } = new(1, "brief", "app.one",
    ...
    private sealed class FakePreferences : IHomeLibraryPreferences
    {
        public Task<bool> IsPinnedAsync(string ownerApp, string artifactId, CancellationToken cancellationToken) => Task.FromResult(false);
    ...
    {
        public HomeDeepLink? Opened { get; private set; }
        public bool CanOpen(HomeDeepLink link) => true;
        public bool Open(HomeDeepLink link) { Opened = link; return true; }
    }
