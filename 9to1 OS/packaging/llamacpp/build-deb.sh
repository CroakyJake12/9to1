...
for file in broker.py gguf.py model_lease.py modelctl.py backend_matrix.py hardware_probe.py budget.py benchmark.py; do
  install -m 0644 "$REPO_ROOT/../9to1 Workspace/Home/Source/Dulche/llamacpp/$file" "$STAGE/usr/lib/haven/inference/$file"
done
install -m 0644 "$REPO_ROOT/../9to1 Workspace/Home/Source/Dulche/llamacpp/provider-contract.json" "$STAGE/usr/lib/haven/inference/provider-contract.json"
install -m 0644 "$REPO_ROOT/../9to1 Workspace/Home/Source/Dulche/llamacpp/backend-matrix.json" "$STAGE/usr/lib/haven/inference/backend-matrix.json"
...
install -m 0644 "$REPO_ROOT/../9to1 Workspace/Home/Source/Dulche/llamacpp/licenses/llama.cpp-LICENSE" "$STAGE/usr/share/doc/$PACKAGE/llama.cpp-LICENSE"
install -m 0644 "$REPO_ROOT/../9to1 Workspace/Home/Source/Dulche/llamacpp/upstream.lock.json" "$STAGE/usr/share/doc/$PACKAGE/upstream.lock.json"
