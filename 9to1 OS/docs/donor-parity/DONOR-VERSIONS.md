...
## 2026-09-24 canonical donor-source continuation

The preceding sections describe the 2026-09-16 historical audit, **not** the current donor-source state. `eng/donor-sources.json` now freezes the live default-branch upstream commits selected on 2026-09-24 for AppFlowy Board, Rnote, Firefox/Gecko, LibreOffice, DuckDB, EDS, libical, libvterm, VSCodium/Code-OSS, glycin/Loupe, GStreamer/GES, llama.cpp, Wine and WinBoat. The controlled `CroakyJake12` forks matched these upstream HEADs when selected, and every consuming app/component has its own detached source checkout under `Source/` plus `Source/DONOR-PROVENANCE.md`. Existing Rnote v0.14.2, DuckDB 1.5.5 and Dulche v0.4.0 runtime pins have **not** been silently replaced with latest-source runtime claims.

The existing Files source was matched to `files-community/Files@21d407b51dbc2b1bd2733ef615d727345d5dd16f` via exact `src` and `tests` tree hashes; upstream repository metadata files excluded in the earlier import are listed in `Files/Source/DONOR-PROVENANCE.md`. These are working-tree materialisation checks, **not** recursive-checkout acceptance while donor gitlinks remain uncommitted. Run `python eng/check-donor-sources.py` for the strict gate.
